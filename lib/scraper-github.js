...
class GraalDefinition extends Definition {
  constructor (props) {
    super(props)
    this.version = this.version.replace(/^vm-/, '')
    this.packageName = `${this.name}+ce-${this.version}`
...
module.exports = class GithubScraper extends Scraper {
  ...
  build (release) {
    return Promise.all(
      release.binaries.map(b =>
        digest(b.apiUrl, {
          headers: {
            Accept: `${b.content_type};q=1.0, application/octet-stream;q=0.9`
          }
        })
          .then(sha => ({ ...b, sha }))
